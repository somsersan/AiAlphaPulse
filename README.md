...
- Структура: `articles` (id, title, content, links_json, link, source, published, etc.)
- Для каждой статьи дополнительно сохраняет извлеченные из текста ссылки в поле `links_json` (JSON-массив объектов `{href, text}`)
...
Если у вас уже создана таблица `articles`, добавьте новый столбец для ссылок:

```sql
ALTER TABLE articles ADD COLUMN IF NOT EXISTS links_json TEXT;
```

Для чистой установки запуск `init_db.py` или `create_all_tables` создаст столбец автоматически.

...
- **Аналитические карточки**: компактные, структурированные отчёты с TL;DR, фактами, sentiment, рекомендациями и уровнем уверенности
- **Telegram бот**: команды `/top`, `/latest`, подписки на горячие новости с детальной аналитикой
...
Используется в pipeline для массовой оценки важности новостей.

Рекомендуемые варианты:
- `anthropic/claude-3.5-haiku` ⭐ (лучший выбор)
- `meta-llama/llama-3.3-70b-instruct` (бесплатная)

### `LLM_ANALYSIS_MODEL` - для генерации аналитических карточек (мощная) 

Используется в Telegram боте для создания детальных аналитических карточек с TL;DR, фактами, sentiment, рекомендациями и уровнем уверенности.
...
### 📊 Формат аналитических карточек

Для каждой новости бот генерирует компактную аналитическую карточку с полями:

- **🔎 TL;DR** (20-30 слов): Суть новости и её влияние на рынки/активы
- **📌 Ключевые факты** (2-4 пункта): Конкретные факты из текста, без домыслов
- **📈 Затронутые активы**: Список тикеров через запятую
- **💡 Sentiment score**: Число от -1 до 1 с кратким объяснением
- **⭐ News score**: Число от 0 до 1 + основные драйверы (sentiment / mentions / authority)
- **🧭 Рекомендация**: "Monitor" / "Bullish (consider buy)" / "Bearish (consider sell)" / "No action" + объяснение
- **🔒 Confidence**: "Low" / "Medium" / "High" + обоснование уверенности
- **🔗 URL**: Ссылка на источник

**Особенности:**
- Язык карточки автоматически определяется по заголовку новости (русский/английский)
- Для горячих новостей (hotness ≥ 0.7) добавляется заголовок алерта с timeline
- Карточки не содержат прямых финансовых советов, используются формулировки: "consider", "monitor", "may indicate"
- При недостатке данных это явно указывается в TL;DR и Confidence

**Пример тестирования:**

```bash
python test_analysis_cards.py
```
